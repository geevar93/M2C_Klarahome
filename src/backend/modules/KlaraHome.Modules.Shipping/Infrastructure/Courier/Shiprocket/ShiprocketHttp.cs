...
internal static class ShiprocketHttp
{
    /// <summary>The named <see cref="HttpClient"/> the adapter resolves.</summary>
    /// <remarks>
    /// Named for the job rather than for the courier. A second adapter books through the same
    /// allow-listed client, and renaming it per courier would mean a new registration for every one.
    /// </remarks>
    public const string ClientName = "shipping-courier";
}

/// <summary>
/// Refuses any outbound request to a host other than the configured courier's.
/// </summary>
/// <remarks>
/// <para>
...
/// courier is chosen per deployment, so the list here is derived from the configured base URL and
/// nothing else. That keeps the guarantee — one host, from configuration only — without pretending
/// to know in advance which courier a client will use.
/// </para>
/// <para>
/// Every request this client makes carries the courier's bearer token, so a misdirected one would
...
internal sealed partial class ShiprocketAllowedHostHandler(
    IOptionsMonitor<ShippingOptions> options,
    ILogger<ShiprocketAllowedHostHandler> logger) : DelegatingHandler
{
    /// <inheritdoc />
...
/// <summary>
/// The webhook origin checks, and the unit conversions a courier's API needs.
/// </summary>
/// <remarks>
/// <para>
...
/// parcel silently becomes ten times heavier. Couriers in India quote weight in <em>kilograms</em>
...
/// </para>
/// </remarks>
internal static class ShiprocketWire
{
    /// <summary>
    /// Whether an offered header value is the configured shared secret itself.
    /// </summary>
    /// <remarks>
    /// Shiprocket's proof of origin is a secret in a header rather than a signature over the body
    /// (docs/08-integrations.md §2.2), which makes the constant-time comparison matter more rather
    /// than less: the value being compared is the credential, not a digest of one.
    /// </remarks>
    /// <param name="offered">The header value that arrived.</param>
    /// <param name="secret">The configured shared secret.</param>
    public static bool MatchesSecret(string? offered, string? secret)
    {
        if (string.IsNullOrWhiteSpace(offered) || string.IsNullOrWhiteSpace(secret))
        {
            return false;
        }

        var candidate = Encoding.UTF8.GetBytes(offered.Trim());
        var expected = Encoding.UTF8.GetBytes(secret.Trim());
